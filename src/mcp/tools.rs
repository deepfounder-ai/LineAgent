...
pub fn list_tools() -> Vec<Tool> {
    vec![
        // ── Tickets ────────────────────────────────────────────────────────
        tool("create_ticket", "Create a new ticket in a project.", json!({
            "type": "object",
            "properties": {
        ...
        tool("update_ticket", "Update an existing ticket.", json!({
            "type": "object",
            "properties": {
        ...
        tool("get_ticket", "Get a single ticket with its comments and relations.", json!({
            "type": "object",
            "properties": {
                "identifier": { "type": "string", "description": "Ticket identifier (e.g. LIN-1)." }
            },
        ...
        tool("list_tickets", "List tickets with optional filters.", json!({
            "type": "object",
            "properties": {
        ...
        tool("delete_ticket", "Delete a ticket. Idempotent — no error if already absent.", json!({
            "type": "object",
            "properties": {
                "identifier": { "type": "string", "description": "Ticket identifier (e.g. LIN-1)." }
            },
        ...
        // ── Comments ───────────────────────────────────────────────────────
        tool("add_comment", "Add a comment to a ticket.", json!({
            "type": "object",
            "properties": {
        ...
        tool("list_comments", "List all comments for a ticket.", json!({
            "type": "object",
            "properties": {
                "ticket_identifier": { "type": "string", "description": "Ticket identifier (e.g. LIN-1)." }
            },
        ...
        // ── Relations ──────────────────────────────────────────────────────
        tool("add_relation", "Add a directed relation between two tickets.", json!({
            "type": "object",
            "properties": {
        ...
        tool("remove_relation", "Remove a relation by its id.", json!({
            "type": "object",
            "properties": {
        ...
        // ── Search ─────────────────────────────────────────────────────────
        tool("search_tickets", "Full-text search over tickets (FTS5 BM25).", json!({
            "type": "object",
            "properties": {
        ...
        tool("get_project", "Get a project by key.", json!({
            "type": "object",
            "properties": {
        ...
        tool("update_project", "Update a project's name and/or description.", json!({
            "type": "object",
            "properties": {
                "key":         { "type": "string", "description": "Project key (e.g. LIN)." },
                "name":        { "type": "string" },
        ...
        tool("list_projects", "List all projects for the current user.", json!({
            "type": "object",
            "properties": {},
            "additionalProperties": false
        })),
        ...
        // ── Cycles ─────────────────────────────────────────────────────────
        tool("create_cycle", "Create a new cycle (sprint) in a project.", json!({
            "type": "object",
            "properties": {
                "project_key": { "type": "string", "description": "Project key (e.g. LIN)." },
        ...
        tool("update_cycle", "Update a cycle's mutable fields.", json!({
            "type": "object",
            "properties": {
        ...
        tool("list_cycles", "List all cycles for a project.", json!({
            "type": "object",
            "properties": {
                "project_key": { "type": "string", "description": "Project key (e.g. LIN)." }
            },
        ...
        // ── Misc ───────────────────────────────────────────────────────────
        tool("get_log", "Return recent audit-log events. Optional since (RFC3339) and limit.", json!({
            "type": "object",
            "properties": {
        ...
        tool("get_index", "Return an index of all projects with ticket status counts.", json!({
            "type": "object",
            "properties": {},
            "additionalProperties": false
        })),
...
pub async fn call_tool(name: &str, args: Value, ctx: &AuthedContext) -> Result<Vec<TextContent>> {
    ...
    match name {
        ...
        "delete_ticket" => {
            ...
            Ok(vec![TextContent::json(json!({ "ok": true, "identifier": identifier }))])
        }
        ...
        "add_comment" => {
            ...
            let comment = svc.add(user_id, &ticket_identifier, author.as_deref(), &body).await?;
        ...
        "add_relation" => {
            ...
            let relation = svc.add(user_id, &from_identifier, &to_identifier, &relation_type).await?;
        ...
        "remove_relation" => {
            ...
            Ok(vec![TextContent::json(json!({ "ok": true, "relation_id": relation_id }))])
        }
        ...
        "create_project" => {
            ...
            let project = svc.create(user_id, &key, &name, description.as_deref()).await?;
        ...
        "update_project" => {
            ...
            let description = arg_str_opt(&args, "description");
            let svc = ProjectService::new(state);
            let project = svc.update(user_id, &key, name.as_deref(), description.as_deref()).await?;
            Ok(vec![TextContent::json(project)])
        }
        ...
        "create_cycle" => {
            ...
            let cycle = svc
                .create(user_id, &project_key, &name, starts_at.as_deref(), ends_at.as_deref())
                .await?;
        ...
        "update_cycle" => {
            ...
            let svc = CycleService::new(state);
            let cycle = svc
                .update(user_id, &cycle_id, name.as_deref(), starts_at.as_deref(), ends_at.as_deref())
                .await?;
            Ok(vec![TextContent::json(cycle)])
        }
        ...
        "get_log" => {
            ...
            let filter = event_repo::EventFilter { since: since_dt, limit };
